...
<body>
	...
	int num_attribute=9;
	int num_photo = f_rs.size()/num_attribute;
	...
	<div><br><br>
		<table class="table" style="width: 700px" align="center">
			<tbody>
				<tr>
					<% if (!contact.isEmpty()) {
							out.println("<td><h3>" + contact.get(1) + "'s Favorites (" + num_photo + " photos)</h3>");
							out.println("<br>"+contact.get(4)); //description
							out.println("<br>" + contact.get(1) + "'s - <a href='friends?uid="+contact.get(0)+"'>Friends("+con_friend.size()+")</a>   <a href='contact?uid="+uid+"&cid="+contact.get(0)+"'>Photos(" + p_rs.size()/num_attribute +")</a></td>");
				...
				if (!contact.isEmpty()) {
					if (f_rs.isEmpty()) {
						out.println("<tr>");
						out.println("<td>" + contact.get(1) + " has no Favorites!</td>");
						out.println("<td></td>");
						out.println("</tr>");
					}
					else {
						...
						if (ps == 4) {
							out.println("    <i class='icon-list'></i> 4 <a href='/favorites?uid="+uid+"&cid="+contact.get(0)+"&pg=1&ps=10'>10</a> <a href='/favorites?uid="+uid+"&cid="+contact.get(0)+"&pg=1&ps=20'>20</a>");
						}
						else if (ps == 10) {
							out.println("    <i class='icon-list'></i> <a href='/favorites?uid="+uid+"&cid="+contact.get(0)+"&pg=1&ps=4'>4</a> 10 <a href='/favorites?uid="+uid+"&cid="+contact.get(0)+"&pg=1&ps=20'>20</a>");
						}
						else if (ps == 20) {
							out.println("    <i class='icon-list'></i> <a href='/favorites?uid="+uid+"&cid="+contact.get(0)+"&pg=1&ps=4'>4</a> <a href='/favorites?uid="+uid+"&cid="+contact.get(0)+"&pg=1&ps=10'>10</a> 20");
						}
						...
						for (int i=0; i<f_rs.size(); i+=num_attribute) {
							out.println("<tr>");
							//display image here
							out.println("<td><ul class='thumbnails'><li class='span4'><div><a href='ServeImage.jsp?pid=" + f_rs.get(i) + "' class='thumbnail'><img src='/imageServe?key="+f_rs.get(i)+"' alt=''></a>");
							//out.println("<td><a href='/photo?pid=" + p_rs.get(i) + "'><img src='/imageServe?key="+p_rs.get(i)+"'></a>");
							out.println("<strong><a href='ServeImage.jsp?pid=" + f_rs.get(i) + "'>" + f_rs.get(i+2) + "</a></strong>");
							if (f_rs.get(i+5) != "") {
								out.println("<br>" + f_rs.get(i+5));	
							}
							out.println("<br><small>Uploaded on " + f_rs.get(i+7));
							out.println("<br><i class='icon-thumbs-up'></i>" + f_rs.get(i+3));
							out.println("  <i class='icon-thumbs-down'></i>" + f_rs.get(i+4));
							out.println("  <i class='icon-comment'></i>" + f_rs.get(i+8) + "</small></div></li></ul></td>");
							...
							if (i<f_rs.size()) {
								...
								out.println("<br><i class='icon-thumbs-up'></i>" + f_rs.get(i+3));
								out.println("  <i class='icon-thumbs-down'></i>" + f_rs.get(i+4));
								out.println("  <i class='icon-comment'></i>" + f_rs.get(i+8) + "</small></div></li></ul></td></tr>");
							}
							else {
